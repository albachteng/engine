...
VoronoiMapScene::VoronoiMapScene(sf::RenderWindow &window) {
    ...
    m_noiseGen = std::make_unique<NoiseGenerator>(m_config.seed);
    m_distorter = std::make_unique<BoundaryDistorter>(m_config.seed);
    
    LOG_INFO_STREAM("VoronoiMapScene: Initialized with window size " << m_window_size.x << "x" << m_window_size.y);
}
...
void VoronoiMapScene::onLoad() {
    ...
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::W}] = VoronoiMapActions::UP;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::A}] = VoronoiMapActions::LEFT;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::S}] = VoronoiMapActions::DOWN;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::D}] = VoronoiMapActions::RIGHT;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::Up}] = VoronoiMapActions::UP;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::Left}] = VoronoiMapActions::LEFT;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::Down}] = VoronoiMapActions::DOWN;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::Right}] = VoronoiMapActions::RIGHT;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::Enter}] = VoronoiMapActions::SELECT;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::R}] = VoronoiMapActions::REGENERATE;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::B}] = VoronoiMapActions::TOGGLE_BOUNDARIES;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::C}] = VoronoiMapActions::TOGGLE_CENTERS;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::T}] = VoronoiMapActions::CYCLE_STYLE;
    m_inputMap[InputEvent{InputType::Keyboard, sf::Keyboard::Escape}] = VoronoiMapActions::BACK_TO_MENU;
    
    // Register input listeners
    m_actionController->registerListener(VoronoiMapActions::UP, 
        [this](float) { navigateInDirection(Direction::UP); });
    m_actionController->registerListener(VoronoiMapActions::LEFT, 
        [this](float) { navigateInDirection(Direction::LEFT); });
    m_actionController->registerListener(VoronoiMapActions::DOWN, 
        [this](float) { navigateInDirection(Direction::DOWN); });
    m_actionController->registerListener(VoronoiMapActions::RIGHT, 
        [this](float) { navigateInDirection(Direction::RIGHT); });
    m_actionController->registerListener(VoronoiMapActions::REGENERATE, 
        [this](float) { regenerateWithNewSeed(); });
    m_actionController->registerListener(VoronoiMapActions::TOGGLE_BOUNDARIES, 
        [this](float) { toggleBoundaryDisplay(); });
    m_actionController->registerListener(VoronoiMapActions::TOGGLE_CENTERS, 
        [this](float) { toggleCenterDisplay(); });
    m_actionController->registerListener(VoronoiMapActions::CYCLE_STYLE, 
        [this](float) { cycleVisualStyle(); });
...
void VoronoiMapScene::update(float deltaTime) { 
...
void VoronoiMapScene::generateVoronoiMap() {
    ...
    m_voronoiGen->clear();
    m_voronoiGen->generateRandomSites(m_config.regionCount, m_config.minRegionDistance, m_config.seed);
    ...
    LOG_INFO_STREAM("VoronoiMapScene: Generated " << cells.size() << " Voronoi cells");
    
    // Create entities for each cell
    for (const auto& cell : cells) {
        auto entity = m_entityManager.addEntity(EntityTag::MAP_NODE); // Reuse existing tag
        
        // Create Voronoi region component
        auto regionComp = CVoronoiRegion(cell.cellId, cell.centroid, cell.vertices);
        
        // Apply fantasy theming
        applyFantasyTheme();
        
        // Distort boundaries if enabled
        if (m_config.boundaryRoughness > 0.0f) {
            regionComp.distortedBoundary = m_distorter->distortBoundary(
                cell.vertices, cell.centroid, 
                m_config.boundaryRoughness, m_config.noiseFrequency, 
                m_config.boundarySubdivisions);
        } else {
        ...
        // Add complex shape for rendering
        sf::Color regionColor = (m_config.useFantasyColors) ? 
            getFantasyColor(cell.cellId) : sf::Color(100 + (cell.cellId * 30) % 155, 
                                                    150 + (cell.cellId * 50) % 105,
                                                    200 - (cell.cellId * 40) % 100);
        
        auto shapeComp = CComplexShape(regionComp.distortedBoundary, regionColor, sf::Color::Black, 2.0f);
    ...
    LOG_INFO_STREAM("VoronoiMapScene: Created " << m_regionIds.size() << " region entities");
}
...
void VoronoiMapScene::regenerateWithNewSeed() {
    ...
    m_distorter = std::make_unique<BoundaryDistorter>(m_config.seed);
    generateVoronoiMap();
    LOG_INFO_STREAM("VoronoiMapScene: Regenerated map with new seed " << m_config.seed);
}

void VoronoiMapScene::applyFantasyTheme() {
    // This will be implemented with fantasy colors and names
...
void VoronoiMapScene::navigateInDirection(Direction dir) {
    ...
    if (nextRegionId != -1 && nextRegionId != m_currentRegionId) {
        ...
        LOG_DEBUG_STREAM("VoronoiMapScene: Navigated to region " << nextRegionId << " in direction " << static_cast<int>(dir));
    } else {
        LOG_DEBUG_STREAM("VoronoiMapScene: No valid region found in direction " << static_cast<int>(dir));
    }
}

int VoronoiMapScene::findClosestRegionInDirection(int currentRegionId, Direction dir) {
    ...
    for (auto &e : m_entityManager.getEntities(EntityTag::MAP_NODE)) {
        if (e->has<CVoronoiRegion>()) {
            auto &region = e->get<CVoronoiRegion>();
            if (region.regionId == currentRegionId || !region.isNavigable) continue;
            
            float score = calculateRegionDirectionalScore(currentPos, region.centroid, dir);
...
float VoronoiMapScene::calculateRegionDirectionalScore(const Vec2f& from, const Vec2f& to, Direction dir) {
    ...
    for (auto &e : m_entityManager.getEntities(EntityTag::MAP_NODE)) {
        if (e->has<CVoronoiRegion>() && e->has<CComplexShape>()) {
            ...
            if (region.isSelected) {
                ...
                float pulseValue = (sin(region.pulseTimer) + 1.0f) * 0.5f;
                ...
                sf::Color pulseColor = region.selectedColor;
                pulseColor.a = alpha;
                shape.fillColor = pulseColor;
...
void VoronoiMapScene::toggleBoundaryDisplay() {
    ...
    LOG_INFO_STREAM("VoronoiMapScene: Boundary display " << (m_showBoundaries ? "enabled" : "disabled"));
}

void VoronoiMapScene::toggleCenterDisplay() {
    m_showCenters = !m_showCenters;
    LOG_INFO_STREAM("VoronoiMapScene: Center display " << (m_showCenters ? "enabled" : "disabled"));
}

void VoronoiMapScene::cycleVisualStyle() {
    m_config.useFantasyColors = !m_config.useFantasyColors;
    generateVoronoiMap(); // Regenerate with new colors
    LOG_INFO_STREAM("VoronoiMapScene: Switched to " << (m_config.useFantasyColors ? "fantasy" : "standard") << " colors");
}